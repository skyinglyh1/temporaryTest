...
TOTAL_ONT_TO_BE_ADD_AT_FILL_KEY = "TotalOntToBeAddAtFill"
...
def Main(operation, args):
    ...
    # if operation == "setCommissionPercentage":
    #     if len(args) != 1:
    #         return False
    #     commissionPercentage = args[0]
    #     return setCommissionPercentage(commissionPercentage)
    if operation == "withdrawGas":
        return withdrawGas()
    if operation == "withdrawCommission":
        return withdrawCommission()
    if operation == "addOntToCurrentRoundAwardVault":
        if len(args) != 1:
            return False
        ontAmount = args[0]
    ...
    # Global Info Start
    if operation == "getTotalONTAmount":
        return getTotalONTAmount()
    ...
    while fillPaperBalanceNeedtoUpdateLeft > 0:
        ...
        if roundPaperBalance != 0:
            if fillPaperBalanceNeedtoUpdateLeft >= roundPaperBalance:
                ...
                awardVaultToBeAdd = Div(Mul(Mul(roundPaperBalance, ONTAssumedMagnitude), awardPercentageAtFill), 100)
                ...
                # get award percentate at fill at fillPaperFromRound round
                awardPercentageAtFill = getAwardAtFillPercentage(fillPaperFromRound)
                awardVaultToBeAdd = Div(Mul(Mul(fillPaperBalanceNeedtoUpdateLeft, ONTAssumedMagnitude), awardPercentageAtFill), 100)
                awardVaultKey = concatKey(concatKey(ROUND_PREFIX, currentRound), AWARD_VAULT_KEY)
                # update current round award vault
                Put(GetContext(), awardVaultKey, Add(awardVaultToBeAdd, getAwardVault(currentRound)))
...
def addOntToCurrentRoundAwardVault(ontAmount):
    ...
    if res == False:
        # , "Add ont to current round award vault failed!"
...
def migrateContract(code, needStorage, name, version, author, email, description, newContractHash):
    ...
    if res == True:
        ...
        Notify(["MigrateContractError", "transfer ONT to new contract error"])
        return False
...
def buyPaper(account, paperAmount):
    ...
    ontAmount = paperToONT(paperAmount)

    # Require(transferONT(account, ContractAddress, ontAmount))
    res = transferONT(account, ContractAddress, ontAmount)
    if res == False:
        # , "Buyer transfers ONT failed!"
        Notify(["BuyPaperError", 1002])
        return False

    assumedOntAmount = Mul(ontAmount, ONTAssumedMagnitude)
    referralPercentage = getReferralPercentage(currentRound)
    dividend1 = Div(Mul(assumedOntAmount, Add(getHolderPercentage(currentRound), referralPercentage)), 100)
    referral = getReferral(account)
    referralAmount = 0

    if referral:
        referralAmount = Div(Mul(assumedOntAmount, referralPercentage), 100)
    ...
    awardVaultToBeAddAtBuy = Div(Mul(assumedOntAmount, getAwardAtBuyPercentage(currentRound)), 100)
    awardVaultKey = concatKey(concatKey(ROUND_PREFIX, currentRound), AWARD_VAULT_KEY)
    Put(GetContext(), awardVaultKey, Add(awardVaultToBeAddAtBuy, getAwardVault(currentRound)))

    gasVaultToBeAdd = Div(Mul(assumedOntAmount, ExecuteFeePercentage), 100)
    Put(GetContext(), GAS_VAULT_KEY, Add(gasVaultToBeAdd, getGasVault()))

    # update the awardVaultAtFill
    awardVaultToBeAddAtFill = Sub(Sub(Sub(assumedOntAmount, dividend1), awardVaultToBeAddAtBuy), gasVaultToBeAdd)
    Put(GetContext(), TOTAL_ONT_TO_BE_ADD_AT_FILL_KEY, Add(getTotalONTToBeAddAtFill(), awardVaultToBeAddAtFill))
    ...
    # update total ONT
    Put(GetContext(), TOTAL_ONT_KEY, Add(getTotalONTAmount(), assumedOntAmount))
    Notify(["buyPaper", account, ontAmount, paperAmount, GetTime()])
...
def reinvest(account, paperAmount):
    ...
    assumedOntToBeReinvest = Add(Add(dividendBalance, awardBalance), referralBalance)

    # Require(assetToBeReinvest >= ontAmount)
    if assumedOntToBeReinvest <= assumedOntAmount:
        # "Not enough ONT to rebuy papers!",
    ...
    referralAmount = 0
    if referral:
        # referralAmount = Div(Mul(ontAmount, ReferralAwardPercentage), 100)
        referralAmount = Div(Mul(assumedOntAmount, referralPercentage), 100)
        Put(GetContext(), concatKey(REFERRAL_BALANCE_OF_PREFIX, referral), Add(referralAmount, getReferralBalance(referral)))
    dividend = Sub(dividend1, referralAmount)
    ...
    # update the account round paper balance
    roundPaperBalanceKey = concatKey(concatKey(ROUND_PREFIX, currentRound), concatKey(PAPER_BALANCE_ROUND_PREFIX, account))
    Put(GetContext(), roundPaperBalanceKey, Add(getRoundPaperBalance(account, currentRound), paperAmount))
    ...
    ontAmountNeedToBeDeduct = assumedOntAmount
    if ontAmountNeedToBeDeduct >= dividendBalance:
        ontAmountNeedToBeDeduct = Sub(ontAmountNeedToBeDeduct, dividendBalance)
        Delete(GetContext(), concatKey(TOTAL_DIVIDEND_OF_PREFIX, account))
    else:
        Put(GetContext(), concatKey(TOTAL_DIVIDEND_OF_PREFIX, account), Sub(dividendBalance, ontAmountNeedToBeDeduct))
        ontAmountNeedToBeDeduct = 0
    if ontAmountNeedToBeDeduct != 0:
        if ontAmountNeedToBeDeduct >= referralBalance:
    ...
    if ontAmountNeedToBeDeduct != 0:
        if ontAmountNeedToBeDeduct > awardBalance:
            ...
            Put(GetContext(), concatKey(AWARD_BALANCE_OF_PREFFIX, account), Sub(awardBalance, ontAmountNeedToBeDeduct))

    Notify(["reBuyPaper", account, ontAmount, paperAmount, GetTime()])
...
def fillPaper(account, guessNumberList):
    ...
    #     awardVaultToBeAdd = Div(Mul(Mul(guessNumberLen, ONTAssumedMagnitude), 2), 100)
    #     awardVaultKey = concatKey(concatKey(ROUND_PREFIX, currentRound), AWARD_VAULT_KEY)
    #     Put(GetContext(), awardVaultKey, Add(awardVaultToBeAdd, getAwardVault(currentRound)))
    ...
    # update award vault, AwardPercentage = 45
    # awardVaultToBeAdd = Div(Mul(ontAmount, AwardPercentage), 100)
    awardVaultToBeAddAtFill = Div(Mul(Mul(guessNumberLen, ONTAssumedMagnitude), getAwardAtFillPercentage(currentRound)), 100)
    awardVaultKey = concatKey(concatKey(ROUND_PREFIX, currentRound), AWARD_VAULT_KEY)
...
def withdraw(account):
    ...
    # update total ont amount
    Put(GetContext(), TOTAL_ONT_KEY, Sub(getTotalONTAmount(), assumedOntToBeWithdraw))

    Notify(["withdraw", ContractAddress, account, ontAmountToBeWithdrawn, GetTime()])
...
################## Global Info Start #######################
def getTotalONTAmount():
    return Get(GetContext(), TOTAL_ONT_KEY)
...
def paperToONT(paperAmount):
    ontAmount = Mul(InitialPrice, paperAmount)
    return ontAmount
...
def transferONTFromContact(toAcct, amount):
